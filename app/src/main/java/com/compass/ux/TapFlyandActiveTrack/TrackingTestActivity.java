package com.compass.ux.TapFlyandActiveTrack;
...
public class TrackingTestActivity extends DemoBaseActivity implements TextureView.SurfaceTextureListener, View.OnClickListener, View.OnTouchListener, CompoundButton.OnCheckedChangeListener, ActiveTrackMissionOperatorListener {
...
//        调用的isGestureModeEnabled()方法ActiveTrackOperator来检查手势模式是否已启用并更新mGestureModeSw开关的值。
//        接着，调用setGestureModeEnabled()的方法ActiveTrackOperator来设置手势模式，并通过示出在完成回调举杯的消息通知用户
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                break;
//            在这种情况ACTION_UP下，我们首先调用该getActiveTrackRect()方法并传递mSendRectIV变量以将绘图矩形转换为视频流坐标系。
//            接下来，检查的值，isDrawingRect并相应地初始化mActiveTrackMission变量。在这里，我们使用ActiveTrackMode的TRACE模式。
//            此外，调用启动ActiveTrack任务的方法，并通过显示敬酒消息来通知用户。最后，我们隐藏图像视图。startTracking()ActiveTrackOperatormSendRectIV
...
//                setRecommendedConfiguration()方法ActiveTrackOperator来设置推荐的摄像机和云台配置。
...
//                    您可以调用该getTargetRect ()方法以获取RectF对象，该对象是实时视频流视图图像中的矩形，
//                    代表要跟踪的目标。矩形被标准化为[0,1]，其中（0,0）是视频预览的左上角，而（1,1）是右下角。
//
//                    通过调用该setTargetRect(RectF var1)方法，可以设置目标rect对象，它是RectF类对象，
//                    它代表目标的边界框。矩形被标准化为[0,1]，其中（0,0）是视频预览的左上角，而（1,1）是右下角
...
//        mTrackingImage根据当前目标rect 更新图像视图的位置和大小。
//        另外，根据来更新mTrackingImage变量的图像资源ActiveTrackTargetState
     ...
     * @return
     */
//    将Android View坐标系的跟踪矩形转换为视频流坐标系
