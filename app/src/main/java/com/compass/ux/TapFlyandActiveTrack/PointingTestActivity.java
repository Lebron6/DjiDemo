...
//                getDirection方法返回一个DJIVector对象，该对象表示3D空间中的笛卡尔矢量。
//                当飞机执行TapFly任务时，您可以使用NED（东北-东北）坐标系统获取飞机的实际飞行方向。
//                该getImageLocation ()方法返回一个PointF对象，该对象表示实时视频流中的图像点，视觉系统应在该视频点上计算飞行方向。
//                图像点被标准化为[0,1]，其中（0,0）是左上角，（1,1）是右下角。
...
//                通过调用setAutoFlightSpeed()方法来设置任务期间飞机的自动飞行速度TapFlyMissionOperator。范围是[1，10] m / s。
//                调用setAutoFlightSpeed()的方法TapFlyMissionOperator中onStopTrackingTouch()
//                的方法setOnSeekBarChangeListener()来配置TapFly任务自动飞行速度。
...
//        如果要在执行TapFly任务时允许飞机绕过障碍物的左侧或右侧绕过障碍物或在障碍物周围移动，
//        可以将此成员变量设置为YES。否则，飞机只会越过障碍物来避免
        mTapFlyMission.isHorizontalObstacleAvoidanceEnabled = mAssisSw.isChecked();
        mTapFlyMission.tapFlyMode = TapFlyMode.FORWARD;

//        FORWARD	飞机将飞向目标。前向障碍物感应系统处于活动状态。
//        BACKWARD	飞机将以与目标相反的方向飞行。向后障碍物感应系统处于活动状态。
//        FREE	飞机将飞向目标。用户可以通过遥控器的操纵杆控制航向。当飞机向侧面飞行时，障碍物传感系统可能无法正常工作。
//        UNKNOWN	TapFly模式未知。
    }

//    我们可以使用该getTapFlyPoint()方法将Android View坐标系的点击点位置转换为视频流坐标系。相比之下，
//    我们使用该showPointByTapFlyPoint()方法将点击位置mRstPointIv从视频流坐标系转换回Android View坐标系并在屏幕上显示。
...
//                        这是我们应该将坐标转换PointF对象传递到的位置，我们可以使用该getTapFlyPoint()方法
//                        进行坐标转换并返回PointF对象。这是视频源中的图像点，视觉系统应从该点计算飞行方向。
